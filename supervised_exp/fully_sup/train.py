...
def get_config():
    ...
    parser.add_argument('--wd',default = 1e-04,type = float, help = 'Weight Decay')
...
def train_n_val(model,optimizer,criterion,train_loader,val_loader,writer,config):
    ...
    for epoch in range(epochs):
        ...
        if(val_acc > best_acc):
            ...
            best_acc = val_acc
            torch.save(model.state_dict(), "results/checkpoints/best_scratch.pth")
...
def main():
    ...
    model.load_state_dict(torch.load("results/checkpoints/best_scratch.pth"))
